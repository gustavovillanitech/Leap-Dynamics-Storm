...
DealForm.setPlayoffOptionRequirements = function (executionContext) {
    ...
    // A deadline and a decision only mean something while an option is still open to be
    // exercised, which is Opt-In and Opt-Out. "In" and "Out" are already settled by the
    // contract: the playoff package is either included or it is not, so there is no date to
    // decide by and no decision to record. "Unknown" has nothing to go on yet.
    //
    // This mirrors the Deal Option rule above, where "No Option" and "Unknown" are exempt for
    // the same reason. Leaving "In" out of this list was blocking deals from being moved to
    // Closed Won over a deadline that does not exist in the contract.
